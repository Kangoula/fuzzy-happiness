(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('bibalDenisApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-bibalDenisApp-error');
                var entityKey = httpResponse.headers('X-bibalDenisApp-params');
                if (errorHeader) {
